...
async function main() {
    ...
    const rows = await prisma.$queryRawUnsafe(`SELECT walletAddress, tokenAddress, signature, COUNT(*) as cnt
     FROM TransferEvent
    ...
    const sigRows = await prisma.$queryRawUnsafe(`SELECT signature, COUNT(*) as cnt
     FROM TransferEvent
     GROUP BY signature
     HAVING cnt > 1
