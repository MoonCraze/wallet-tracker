...
export class WebhookService {
    ...
    constructor() {
        ...
        const walletsData = JSON.parse(readFileSync(new URL("../wallets.json", import.meta.url), "utf-8"));
    ...
    async logWebhookEvent(events, headers) {
        const config = getConfig();
    ...
    async deduplicateAndFilter(transfers) {
        const config = getConfig();
    ...
    async getExistingTransferKeys(transfers) {
        const config = getConfig();
        try {
    ...
    async saveTransfers(transfers) {
        ...
        try {
            ...
            return result?.count ?? transfers.length;
        }
        catch (error) {
    ...
    async checkCoordinatedTrades(touchedTokens) {
        const config = getConfig();
        const now = new Date();
    ...
    async processTokenForCoordination(tokenAddress, windowStart, windowEnd, triggeredAt) {
        const config = getConfig();
