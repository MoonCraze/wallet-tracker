...
if (ALLOW_DEV_ENDPOINTS) {
  ...
  app.post("/dev/transfers", express.json(), (req, res) => {
    ...
    try { publishTransfers(Array.isArray(req.body) && req.body.length ? req.body : sample); } catch {}
...
app.post(
    ...
    for (const evt of events) {
      ...
      const filtered = parsed.filter((p) => {
        if (EXCLUDE_TOKENS().has(p.tokenAddress)) {
          if (DEBUG_EVENTS_VERBOSE()) console.log("[filter] exclude token", { tokenAddress: p.tokenAddress, signature: p.signature });
          return false;
        }
        const amt = Math.abs(Number(p.amount));
        if (!Number.isFinite(amt) || amt < MIN_AMOUNT()) {
          if (DEBUG_EVENTS_VERBOSE()) console.log("[filter] below min amount", { amount: p.amount, min: MIN_AMOUNT(), signature: p.signature });
          return false;
        }
        return true;
      });
      ...
      const uniqMap = new Map<string, typeof filtered[number]>();
      ...
      try {
        ...
        if (DEBUG_EVENTS()) console.warn("[db] existing keys check failed", e);
      }
  ...
  if (newRows.length > 0) {
      ...
      if (DEBUG_EVENTS()) console.log(`[webhook] inserted: ${created}/${toWrite.length} (skipped existing: ${toWrite.length - created})`);
      ...
      if (newRows.length > 0) {
        ...
        try { publishTransfers(broadcastRows); } catch (e) { if (DEBUG_EVENTS()) console.warn("[sse] transfer broadcast failed", e); }
      }

      for (const p of newRows) if (p.side === "BUY") touchedTokens.add(p.tokenAddress);
    }
    ...
    // Check coordinated buys per touched token using fixed time windows
    const now = new Date();
    ...
    if (DEBUG_EVENTS()) {
      console.log(`[coord] checking window: ${windowStart.toISOString()} to ${windowEnd.toISOString()}`);
    }
    
    for (const tokenAddress of touchedTokens) {
      const rows = await prisma.transferEvent.findMany({ 
        where: { tokenAddress, side: "BUY", timestamp: { gte: windowStart, lt: windowEnd } }, 
        select: { walletAddress: true } 
      });
      const uniq = Array.from(new Set(rows.map(r => r.walletAddress))).sort();
      if (uniq.length >= COORDINATED_MIN_WALLETS()) {
        try {
          // Check if this exact window was already processed for this token
          ...
          if (existing) {
            if (DEBUG_EVENTS()) {
              console.log(`[coord] token=${tokenAddress} already processed for window ${windowStart.toISOString()}, skipping`);
            }
            continue;
          }
          
          const created = await prisma.coordinatedTrade.create({ 
            data: { tokenAddress, windowStart, windowEnd, triggeredAt: now, uniqueWalletCount: uniq.length, walletAddresses: JSON.stringify(uniq) } 
          });
          try { 
            publishCoordinated({ 
              tokenAddress, 
            ...
            if (DEBUG_EVENTS()) console.warn("[sse] coordinated broadcast failed", e); 
          }
          if (DEBUG_EVENTS()) console.log("[coord] detected coordinated BUY:", { token: tokenAddress, windowStart: windowStart.toISOString(), count: uniq.length });
        } catch (e) {
...
server.listen(PORT, () => {
  ...
  const scan = async () => {
    try {
      ...
      if (DEBUG_EVENTS()) {
        console.log(`[coord/bg] scanning window: ${windowStart.toISOString()} to ${windowEnd.toISOString()}`);
      }
      ...
      if (existingWindowCheck) {
        if (DEBUG_EVENTS()) {
          console.log(`[coord/bg] window already processed, skipping`);
        }
        return;
      }
      ...
      if (DEBUG_EVENTS() && tokenRows.length > 0) {
        console.log(`[coord/bg] found ${tokenRows.length} unique tokens with BUY activity`);
      }
      
      for (const tr of tokenRows) {
        const token = tr.tokenAddress;
        const buyers = await prisma.transferEvent.findMany({
          where: { tokenAddress: token, side: "BUY", timestamp: { gte: windowStart, lt: windowEnd } },
          select: { walletAddress: true },
        ...
        if (uniq.length >= COORDINATED_MIN_WALLETS()) {
          ...
          if (existing) {
            if (DEBUG_EVENTS()) {
              console.log(`[coord/bg] token=${token} already has entry for this window, skipping`);
            }
            continue;
          }
