      ...
      if (uniq.length >= COORDINATED_MIN_WALLETS()) {
        try {
          ...
          if (existing) {
            ...
            try { publishCoordinated({ tokenAddress, windowStart: windowStart.toISOString(), windowEnd: windowEnd.toISOString(), triggeredAt: now.toISOString(), uniqueWalletCount: updated.uniqueWalletCount, walletAddresses: JSON.parse(updated.walletAddresses || "[]") }); } catch (e) { if (DEBUG_EVENTS()) console.warn("[sse] coordinated broadcast failed", e); }
          } else {
            ...
            try { publishCoordinated({ tokenAddress, windowStart: windowStart.toISOString(), windowEnd: windowEnd.toISOString(), triggeredAt: now.toISOString(), uniqueWalletCount: created.uniqueWalletCount, walletAddresses: JSON.parse(created.walletAddresses || "[]") }); } catch (e) { if (DEBUG_EVENTS()) console.warn("[sse] coordinated broadcast failed", e); }
          }
